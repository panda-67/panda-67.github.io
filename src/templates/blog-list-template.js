...
export default class IndexBlog extends React.Component {
  render() {
    ...
    const tags = this.props.data.tagsPosts.group.map(
      (tag) => <TagsLink key={tag.fieldValue} tag={tag} />
    )
      ...
      <Frame path={path}>
        ...
        <div className="flex flex-col-reverse lg:grid grid-cols-10 mx-6 lg:mx-10 lg:px-6 mt-4">
          ...
          <section className="col-span-7 lg:-ml-16">
            {posts.filter((edge) => !!edge.node.frontmatter.date).map(
              (edge) => <PostLink key={edge.node.id} post={edge.node} />
            )}
